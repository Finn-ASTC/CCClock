...
extern void clk_key_io2_test_inject(clk_key_event2 ev);
extern void clk_key_io2_test_inject_raw(int ch);
extern void clk_key_io2_test_queue_byte(int ch);
extern void clk_key_io2_test_reset(void);
extern void clk_key_io2_test_pause(void);
extern void clk_key_io2_test_resume(void);
...
int main(void) {
    ...
    clk_key_io2_init();
    TEST("clk_key_io2_init() no crash", 1);
    clk_key_io2_close();
    TEST("clk_key_io2_close() no crash", 1);
    ...
    clk_key_io2_init();
    clk_key_io2_set_input(buf, 32, &len, &pos);
    ...
    clk_key_io2_set_input(NULL, 32, &len, &pos);
    TEST("set_input NULL buf safe", 1);
    clk_key_io2_set_input(buf, 0, &len, &pos);
    TEST("set_input zero max_chars safe", 1);
    clk_key_io2_set_input(buf, 32, NULL, &pos);
    TEST("set_input NULL len safe", 1);
    clk_key_io2_set_input(buf, 32, &len, NULL);
    ...
    clk_key_io2_init();
    {
        clk_key_event2 ev1 = {.key_mask = KEY_ESC};
        clk_key_event2 ev2 = {.key_mask = MOD_CTRL | KEY_A_UPPER};

        clk_key_io2_test_inject(ev1);
        clk_key_io2_test_inject(ev2);

        clk_key_event2 got1 = clk_normal_get_key_event2();
        TEST("inject ESC → key_mask == KEY_ESC", got1.key_mask == KEY_ESC);

        clk_key_event2 got2 = clk_normal_get_key_event2();
        TEST("inject Ctrl+A → mod+key", got2.key_mask == (MOD_CTRL | KEY_A_UPPER));

        clk_key_event2 got3 = clk_normal_get_key_event2();
    ...
    clk_key_io2_set_input(buf, 32, &len, &pos);
    {
        clk_key_event2 tev = {
            .key_mask = KEY_a_LOWER, .text = {'a', 0}, .text_len = 1, .has_text = true};
        clk_key_io2_test_inject(tev);

        clk_key_event2 got = clk_input_get_key_event2();
    ...
    {
        clk_key_event2 e = {.key_mask = MOD_CTRL | KEY_S_UPPER};
        TEST("key_is2 Ctrl+S(+) ", clk_key_is2(e, KEY_S_UPPER, MOD_CTRL));
        TEST("key_is2 Ctrl+S(−) wrong key", !clk_key_is2(e, KEY_T_UPPER, MOD_CTRL));
        TEST("key_is2 Ctrl+S(−) wrong mod", !clk_key_is2(e, KEY_S_UPPER, MOD_META));
        TEST("key_is2 Ctrl+S(−) no mod", !clk_key_is2(e, KEY_S_UPPER, 0));
    }
    {
        clk_key_event2 e = {.key_mask = KEY_ESC};
        TEST("key_is2 ESC(+) ", clk_key_is2(e, KEY_ESC, 0));
        TEST("key_is2 ESC(−) stray mod", !clk_key_is2(e, KEY_ESC, MOD_CTRL));
    }
    {
        clk_key_event2 e = {.key_mask = MOD_CTRL | MOD_SHIFT | KEY_J_UPPER};
        TEST("key_is2 Ctrl+Shift+J(+) ", clk_key_is2(e, KEY_J_UPPER, MOD_CTRL | MOD_SHIFT));
        TEST("key_is2 Ctrl+Shift+J(−) missing mod", !clk_key_is2(e, KEY_J_UPPER, MOD_CTRL));
    }
    ...
    clk_key_io2_init();
    clk_key_io2_test_pause();
    clk_key_io2_test_reset();

    /* ---- ASCII printable ---- */
    clk_key_io2_test_inject_raw('a');
    ev = clk_normal_get_key_event2();
    ...
    clk_key_io2_test_inject_raw('0');
    ev = clk_normal_get_key_event2();
    TEST("sm '0' → KEY_0", ev.key_mask == KEY_0);

    clk_key_io2_test_inject_raw(' ');
    ev = clk_normal_get_key_event2();
    ...
    /* ---- Ctrl + letter ---- */
    clk_key_io2_test_inject_raw(0x01);
    ev = clk_normal_get_key_event2();
    TEST("sm Ctrl+A → MOD_CTRL|KEY_A_UPPER", ev.key_mask == (MOD_CTRL | KEY_A_UPPER));

    clk_key_io2_test_inject_raw(0x1A);
    ev = clk_normal_get_key_event2();
    TEST("sm Ctrl+Z → MOD_CTRL|KEY_Z_UPPER", ev.key_mask == (MOD_CTRL | KEY_Z_UPPER));

    /* ---- Special keys (same mechanism as ASCII printable above) ---- */
    /* Verified by interaction — the same process_byte path as 'A'/'0'/etc. */

    /* ---- ESC alone (no follow-on) ---- */
    clk_key_io2_test_inject_raw(0x1B);
    ev = clk_normal_get_key_event2();
    TEST("sm ESC → KEY_ESC", ev.key_mask == KEY_ESC);

    /* ---- CSI arrows ---- */
    clk_key_io2_test_queue_byte('[');
    clk_key_io2_test_inject_raw(0x1B);
    clk_key_io2_test_inject_raw('A');
    ev = clk_normal_get_key_event2();
    ...
    clk_key_io2_test_queue_byte('[');
    clk_key_io2_test_inject_raw(0x1B);
    clk_key_io2_test_inject_raw('H');
    ev = clk_normal_get_key_event2();
    TEST("sm CSI H → KEY_HOME", ev.key_mask == KEY_HOME);

    clk_key_io2_test_queue_byte('[');
    clk_key_io2_test_inject_raw(0x1B);
    clk_key_io2_test_inject_raw('F');
    ev = clk_normal_get_key_event2();
    ...
    clk_key_io2_test_queue_byte('[');
    clk_key_io2_test_inject_raw(0x1B);
    clk_key_io2_test_inject_raw('1');
    clk_key_io2_test_inject_raw('5');
    clk_key_io2_test_inject_raw('~');
    ev = clk_normal_get_key_event2();
    ...
    clk_key_io2_test_inject_raw(0xC3);
    ev = clk_input_get_key_event2();
    TEST("sm UTF-8 lead(2b) → no event", ev.key_mask == 0 && !ev.has_text);
    clk_key_io2_test_inject_raw(0xA1);
    ev = clk_input_get_key_event2();
