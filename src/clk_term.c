...
#define APPENDF(buf, cap, len, ...)                          \
    do {                                                     \
        if ((size_t)(len) >= (size_t)(cap))                  \
            return;                                          \
        size_t _rem = (size_t)(cap) - (size_t)(len);         \
        int _n = snprintf((buf) + (len), _rem, __VA_ARGS__); \
        if (_n < 0 || (size_t)_n >= _rem)                    \
            return;                                          \
        (len) += _n;                                         \
    } while (0)
...
bool clk_term_init(void) {
    ...
    clk_cell* screen_buf = malloc((size_t)cell_count * sizeof(clk_cell));
    if (!screen_buf)
        return false;
    ...
    clk_sprite** sprite_buf = malloc((size_t)sprite_capacity * sizeof(clk_sprite*));
    if (!sprite_buf) {
        free(screen_buffer);
        return false;
    }
    ...
    bool* claimed_buf = calloc((size_t)cell_count, sizeof(bool));
    if (!claimed_buf) {
        free(screen_buffer);
    ...
    char* ansi_buf = calloc(1, (size_t)cell_count * CLK_ANSI_OUTPUT_ESTIMATE_PER_CELL);
    if (!ansi_buf) {
        free(screen_buffer);
        free(sprite_render_list);
        free(cell_claimed);
    ...
    clk_style* styles_buf = malloc((size_t)CLK_STYLE_DEFAULT_CAPACITY * sizeof(clk_style));
    if (!styles_buf) {
        free(screen_buffer);
        free(sprite_render_list);
        free(cell_claimed);
...
int clk_term_register_style(clk_color fg, clk_color bg, uint8_t attrs) {
    ...
    if (style_count + 1 > style_capacity) {
        ...
        clk_style* temp = realloc(style_registry, (size_t)new_cap * sizeof(clk_style));
        if (!temp)
            return 0;
        style_registry = temp;
...
static int cmp_sprite_z_order(const void* raw_a, const void* raw_b) {
    const clk_sprite* a = *(const clk_sprite* const*)raw_a;
    const clk_sprite* b = *(const clk_sprite* const*)raw_b;
...
void clk_term_add_sprite(clk_sprite* sprite) {
    ...
    if (count > sprite_capacity) {
        ...
        clk_sprite** temp = realloc(sprite_render_list, (size_t)new_capacity * sizeof(clk_sprite*));
        if (!temp)
            return;
    ...
    if (sprite_count < sprite_capacity)
...
static void clk_add_cell_to_ansi_output(const clk_cell* cell, int x, int y) {
    ...
    if (ansi_output_length + len > ansi_output_capacity) {
        ...
        while (new_cap < ansi_output_length + len)
            new_cap *= 2;
        char* temp = realloc(ansi_output, (size_t)new_cap);
        if (!temp)
            return;
        ansi_output = temp;
    ...
    memcpy(ansi_output + ansi_output_length, buf, (size_t)len);
...
void clk_term_draw(void) {
    ...
    if (!sprite_list_sorted) {
        qsort(sprite_render_list, (size_t)sprite_count, sizeof(clk_sprite*), cmp_sprite_z_order);
        sprite_list_sorted = true;
    }
    ...
    if (ansi_output_length > 0) {
        fwrite(ansi_output, 1, (size_t)ansi_output_length, stdout);
        fflush(stdout);
        ansi_output_length = 0;
    }
...
bool clk_resize_term(int new_w, int new_h) {
    ...
    clk_cell* new_buf = malloc((size_t)new_size * sizeof(clk_cell));
    bool* new_sign = malloc((size_t)new_size * sizeof(bool));
    char* new_ansi = malloc((size_t)new_cap);
    ...
    memset(new_sign, 0, (size_t)new_size * sizeof(bool));
...
clk_texture clk_texture_create(int w, int h) {
    ...
    clk_cell* data = malloc((size_t)w * (size_t)h * sizeof(clk_cell));
    if (!data)
        return tex;
...
/* ================================================================
 *  Unicode character width — Markus Kuhn mk_wcwidth (Public Domain)
 *  Based on Unicode 5.0, with emoji/symbol ranges added from later
...
static int mk_bisearch(wchar_t ucs, const struct mk_interval* table, int max) {
    int min = 0;
    int mid;
...
static int mk_wcwidth(wchar_t ucs) {
    static const struct mk_interval combining[] = {
...
static int clk_cell_char_width(const char* utf8) {
    ...
    return mk_wcwidth((wchar_t)codepoint);
}
