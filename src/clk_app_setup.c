...
static void register_basic_tab(const clk_app_config* cfg, clk_menu* menu) {
    ...
    /* ---- time format ---- */
    clk_menu_add_item_str(menu, CLK_TAB_BASIC, CLK_BASIC_ITEM_TFMT, "time format",
                          cfg->ascii_clock.time_formats.idx, cfg->ascii_clock.time_formats.options,
                          cfg->ascii_clock.time_formats.count);

    /* ---- font ---- */
    clk_menu_add_item_str(menu, CLK_TAB_BASIC, CLK_BASIC_ITEM_FONT, "font",
                          cfg->ascii_clock.fonts.idx, (const char**)cfg->ascii_clock.fonts.names,
                          cfg->ascii_clock.fonts.count);

    /* ---- menu theme ---- */
    clk_menu_add_item_str(menu, CLK_TAB_BASIC, CLK_BASIC_ITEM_THEME, "menu theme", cfg->themes.idx,
...
bool clk_app_setup_render(clk_ascii_render* render, const clk_cfg_ascii_clock_theme* ascii_clock) {
    if (!render || !ascii_clock)
        return false;
    if (ascii_clock->fonts.idx < 0 || ascii_clock->fonts.idx >= ascii_clock->fonts.count)
        return false;

    if (!clk_ascii_render_create(render, ascii_clock->fonts.paths[ascii_clock->fonts.idx]))
        return false;
