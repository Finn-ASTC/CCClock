...
一个字符重复填充。`fg`/`bg`/`attr` 同上。

```jsonc
"line_h": { "type": "fill", "char": "─", "fg": "#888", "bg": "#000" }
```
...
实际填充宽度由 row 引用处的 `fill: N` 锚点控制(见第 4 节)。不写 `fill` 属性时默认填到行尾。
...
| 规则 | 说明 |
|------|------|
| 含义 | 绝对位置(占总行宽的百分比),不是增量 |
| 递增 | `fill` 值必须严格递增,否则解析报错 |
| 末位 | 最后一个 fill 可不写 `fill` 属性,默认填到行尾 |
| 越界 | 若当前位置已超过 `fill` 对应的位置,填 0 格 |
| 可给任何 ref | 叶子/复合/特殊复合 都可用 `fill` |
| 仅 fill 叶子生效 | `ref` 指向 `string` / `tab_name` 等非 fill 叶子时 `fill` 被忽略 |
...
// 左边框 → 填到 10% → tab 区域填到 40% → 空白填到底 → 右边框
```
...
```jsonc
{
    "defs": {
        ...
        "line_h":  { "type": "fill", "char": "─", "fg": "#888", "bg": "#000"             },
        "line_s":  { "type": "fill", "char": "─", "fg": "#444", "bg": "#000"             },
        "blank":   { "type": "fill", "char": " ", "fg": "#000", "bg": "#000"             },
...
| 6 | `fill` 属性只对 `type: "fill"` 的叶子生效,对 string/tab_name 等忽略 |
